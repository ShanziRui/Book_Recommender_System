...
class Book():
    ...
    def getHeadings(self):
        ...
        arabicNumerals = '\d+'
        romanNumerals = '(?=[CLXVI])(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})'
        ...
        ordinalNumberWords = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth',
            ...
            [numberWord + 'th' for numberWord in numberWords] + \
            ordinalNumberWordsByTens
        ordinalsPat = '(the )?(' + '|'.join(ordinalNumberWords) + ')'
        enumeratorsList = [arabicNumerals,
                           romanNumerals, numberWordsPat, ordinalsPat]
    ...
    def getEndLocation(self):
        ...
        if endLocation is None:  # Can't find the ending.
            logging.info(
                "Can't find an ending line. Assuming that the book ends at the end of the text.")
    ...
    def getTextBetweenHeadings(self):
        ...
        for i, headingLocation in enumerate(self.headingLocations):
            if i != lastHeading:
                ...
                chapters.append(
                    self.lines[headingLocation:nextHeadingLocation])
        return chapters
    ...
    def getStats(self):
        ...
        averageChapterLength = sum([len(chapter)
                                    for chapter in self.chapters])/numChapters
